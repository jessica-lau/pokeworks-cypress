...
describe("Why Pokeworks page", () => {
  ...
  context("User views content on Why Pokeworks page", () => {
    ...
    //     .getSectionDescription()
    //     .eq(6)
    //     .should("contain", "500 Ranking");
    // });

    // it("Pokeworks contact form heading and description are visible", () => {
    //   whyPokeworks
    //     .getSectionHeading()
    //     .should("contain", "POKEWORKS FRANCHISING");
    //   whyPokeworks
    ...
    it("Each option in the net worth dropdown can be selected", () => {
      cy.get("select").select("$100,000 - $250,000");
      cy.get("select").select("$250,000 - $500,000");
      cy.get("select").select("$500,000 - $750,000");
      cy.get("select").select("$750,000 - $1,000,000");
      cy.get("select").select("$1,000,000+");
    });
